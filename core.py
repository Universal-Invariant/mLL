...
class NLPProcessor:
    ...
    def batch_stanza_extract(self, stanza_code, sentences):
        ...
        if not all_words or all_words[0].start_char is None:
            return self.sequential_stanza_extract(stanza_code, sentences)
        ...
        for p in sentences:
            ...
            if start == -1:
                start = current_idx
            sentence_offsets.append((start, start + len(p)))
            current_idx = start + len(p) + 2

        sentence_words = [[] for _ in sentences]
        for word in all_words:
            if word.start_char is not None:
                for i, (p_start, p_end) in enumerate(sentence_offsets):
                    if p_start <= word.start_char < p_end:
                        sentence_words[i].append({
    ...
    def sequential_stanza_extract(self, stanza_code, sentences):
        nlp = self.get_stanza_pipeline(stanza_code)
        sentence_words = []
        for s in sentences:
            doc = nlp(s)
            sentence_words.append([{"text": w.text, "lemma": w.lemma, "pos": w.pos, "css": LanguageMapper.POS_CSS_MAP.get(
    ...
    def back_translate_words(self, words, target_lang, source_lang):
        ...
        try:
            ...
            for word in words:
                # Calculate future length (+3 accounts for the " \n " separator)
                if current_length + len(word) + 3 > 4500:
                    # Translate current safe chunk
                    text_bulk = " \n ".join(current_chunk)
                    translated_bulk = GoogleTranslator(source=target_lang, target=source_lang).translate(text_bulk)
                    if translated_bulk:
                        translated_words.extend([w.strip() for w in translated_bulk.split('\n')])
            ...
            # Translate final remaining chunk
            if current_chunk:
                text_bulk = " \n ".join(current_chunk)
                translated_bulk = GoogleTranslator(source=target_lang, target=source_lang).translate(text_bulk)
                if translated_bulk:
